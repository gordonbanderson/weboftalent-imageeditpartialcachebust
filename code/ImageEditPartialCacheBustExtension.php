...
class ImageEditPartialCacheBustExtension extends DataExtension {
	...
	/*
	This is intended to be added to Image as an extension.  When an image is edited  checks are
	made against a configurable list of classes mapped to the image ID field.  If the image ID
	field value matches the  ID of the image being refocussed, the DataObject's LastEdited field
	is updated.
	*/
	public function onAfterWrite() {
			...
			foreach ($sitetreeclasses as $clazz => $idfield) {
				...
				foreach ($objectsWithImage as $objectWithImage) {
					foreach ($stages as $stage) {
						...
						$sql = "UPDATE `SiteTree{$suffix}` SET LastEdited=NOW() where ID=".
							$objectWithImage->ID;
						DB::query($sql);
					}
				}
		...
		if ($dataobjectclasses) {
			...
			foreach ($dataobjectclasses as $clazz => $idfield) {
				...
				$objectsWithImage = $instanceofclass::get()->filter($idfield, $this->owner->ID);
				foreach ($objectsWithImage as $objectWithImage) {
						$sql = "UPDATE `$clazz` SET LastEdited=NOW() where ID=".
							$objectWithImage->ID;
						DB::query($sql);
				}
			}
